...
(N, Mode, T,
    kalmanT, kalmanT_dot,
    ma2T, ma3T, ma5T, ma8T, ma13T, ma21T, ma34T, ma55T, ma89T, ma144T) = np.loadtxt(path.join(inpath, currentfile),
...
skalmanT_dot = np.zeros((len(T[:, 0]), 10))
...
maxvel = np.amax(kalmanT_dot)
minvel = np.amin(kalmanT_dot)
deltaval = np.abs(maxvel - minvel)
skalmanT_dot = (kalmanT_dot - minvel) / (deltaval * 0.5) - 1.0
...
print(np.amax(kalmanT_dot), np.amin(kalmanT_dot), np.mean(kalmanT_dot))
...
print("N;Mode;T_0;T_1;T_2;T_3;T_4;T_5;T_6;T_7;T_8;T_9;kfT_0;kfT_1;kfT_2;kfT_3;kfT_4;kfT_5;kfT_6;kfT_7;kfT_8;kfT_9;kfT_dot_0;kfT_dot_1;kfT_dot_2;kfT_3;kfT_dot_4;kfT_5;kfT_dot_6;kfT_7;kfT_dot_8;kfT_dot_9;ma3T_0;ma3T_1;ma3T_2;ma3T_3;ma3T_4;ma3T_5;ma3T_6;ma3T_7;ma3T_8;ma3T_9;ma13T_0;ma13T_1;ma13T_2;ma13T_3;ma13T_4;ma13T_5;ma13T_6;ma13T_7;ma13T_8;ma13T_9;ma55T_0;ma55T_1;ma55T_2;ma55T_3;ma55T_4;ma55T_5;ma55T_6;ma55T_7;ma55T_8;ma55T_9;ma144T_0;ma144T_1;ma144T_2;ma144T_3;ma144T_4;ma144T_5;ma144T_6;ma144T_7;ma144T_8;ma144T_9;", file=f)
for line in range(0, len(T[:, 0])):
    print(N[line], sMode[line],
          ...
          skalmanT_dot[line, 0], skalmanT_dot[line, 1], skalmanT_dot[line, 2], skalmanT_dot[line, 3], skalmanT_dot[line, 4], skalmanT_dot[line, 5], skalmanT_dot[line, 6], skalmanT_dot[line, 7], skalmanT_dot[line, 8], skalmanT_dot[line, 9],
...
for agmnt_index in range(1, agmntCount + 1):
    ...
    agmntdT[:, 20:30]=skalmanT_dot + np.random.normal(mu, sigma, (np.size(sT[:, 0]), np.size(sT[0, :])))
    agmntdT[:, 30:40]=sma3T + np.random.normal(mu, sigma, (np.size(sT[:, 0]), np.size(sT[0, :])))
    agmntdT[:, 40:50]=sma13T + np.random.normal(mu, sigma, (np.size(sT[:, 0]), np.size(sT[0, :])))
    agmntdT[:, 50:60]=sma55T + np.random.normal(mu, sigma, (np.size(sT[:, 0]), np.size(sT[0, :])))
    agmntdT[:, 60:70]=sma144T + np.random.normal(mu, sigma, (np.size(sT[:, 0]), np.size(sT[0, :])))
    ...
    for line in range(0, len(T[:, 0])):
        print(N[line], sMode[line],
              ...
              # 3agmntdT[line, 100], agmntdT[line, 101], agmntdT[line, 102], agmntdT[line, 103], agmntdT[line, 104], agmntdT[line, 105], agmntdT[line, 106], agmntdT[line, 107], agmntdT[line, 108], agmntdT[line, 109],
