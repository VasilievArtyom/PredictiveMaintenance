...
for outputBlockId in range(5, 10):
	for _predictSteps in range(1, predictSteps + 1):
		...
		model.compile(Adam(learning_rate=5e-4), loss=loss_mse_warmup)
		...
		model.fit_generator(generator=generator, epochs=1000000, steps_per_epoch=800, validation_data=validation_data, callbacks=callbacks)
